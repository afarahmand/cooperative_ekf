...
run_optimization = logical([1; 1; 1]);             % 0 = (diagnostic) Do not change inputs, 1 = change inputs according to cost function
...
mode_output = [1, 1, 0, 1, 1,    1, 0, 1, 0, 0,    0, 0, 0, 0];
%mode_output = ones(14);
...
iteration_to_start_optimization = 20000;                          % KF_iteration to start optimization
safe_distance = [2; 2; 2];                                                        % Safe distance to maintain around each respective vehicle
safety_margin = 0;                                                                 % Additional distance to maintain between vehicles
direction = [0; 0; 0];                                                               % 1 - Left turn, 2 - Straight, 3 - Right turn (Set in vehicle ICs)
...
u(3*car - 2, 10001:10313) = 0;
u(3*car - 0, 1:KF_iterations) = 270;
u(3*car - 1, 10001:10313) = 19;
...
predictions(5*car - 2, 1) = .0033558;
predictions(5*car - 1, 1) = 45.25 * (1609.344/3600) * (1/(h*gear_ratios(car, 1)) );
u(3*car - 2, 1:iteration_to_start_optimization) = 85;
...
predictions(5*car - 3, 1) = -1000;
predictions(5*car - 2, 1) = .0033558;
predictions(5*car - 1, 1) = 45.50 * (1609.344/3600) * (1/(h*gear_ratios(car, 1)) );
u(3*car - 2, 1:iteration_to_start_optimization) = 85;
u(3*car - 0, 1:KF_iterations) = 90;
end
    ...
    if((sum(run_optimization) > 0) & (KF_interval > iteration_to_start_optimization))       % At least one car needs to be optimized then true
        ...
        for i = 1:num_of_cars
            ...
            elseif(direction(i) == 3)
                ...
                if( (v + 0.1) > (speed_limits(2)*1609.344/3600) )         % At speed limit
                    d = max( abs(corrected_states(5*i - 4, KF_interval)), abs(corrected_states(5*i - 3, KF_interval)) ) - safe_distance(i) - braking_distances(3);
                    k(1) = d/(corrected_states(5*i - 1, KF_interval)*gear_ratios(i, KF_interval)*h);
                    k(2) = 0;
                elseif( v > (45*1609.344/3600) )                                      % High 4th Gear
                    % Find position of car after accelerating to speed limit
