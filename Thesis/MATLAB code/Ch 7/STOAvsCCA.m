...
run_optimization = logical([1; 1; 1; 1]);         % 0 = (diagnostic) Do not change inputs, 1 = change inputs according to cost function
...
% Topology Parameters
speed_limits = [2.24; 10; 2.24];            % Speed limits (left turn, straight, right turn)
lane_width = 2; line_separation = .1;

% Used by cost function
iteration_to_start_optimization = 20;                                % KF_iteration to start optimization
...
predictions(5*car - 4, 1) = -20 - lane_width;
...
u(3*car - 2, 1:iteration_to_start_optimization) = steady_speed_t_angles(2);
...
direction(car) = 1;
gear_ratios(car, 1) = Rg(1);
predictions(5*car - 4, 1) = 12 + lane_width;
predictions(5*car - 3, 1) = lane_width/2;
predictions(5*car - 2, 1) = .00033558;
predictions(5*car - 1, 1) = 2 * (1/(h*gear_ratios(car, 1)) );
u(3*car - 2, 1:iteration_to_start_optimization) = steady_speed_t_angles(2);
u(3*car - 0, 1:KF_iterations) = 180;
end
