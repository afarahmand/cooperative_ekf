...
pieces = 6;
KF_iterations = 24*10^4;                % Number of iterations for KF
...
mode_output = [0, 0, 0, 0, 0,    1, 1, 0, 1, 0,    0, 0, 0, 0];
%mode_output = ones(14);
...
lane_width = 5; line_separation = .1;
...
direction = [0; 0];                                                                   % 1 - Left turn, 2 - Straight, 3 - Right turn (Set in vehicle ICs)
...
predictions(5*car - 4, 1) = -400;
predictions(5*car - 3, 1) = -lane_width/2;
predictions(5*car - 2, 1) = .0047;
predictions(5*car - 1, 1) = 2 * (1609.344/3600) *(1/(h*gear_ratios(car, 1)));
...
start = floor([1;       8.71;  23.46;  62.5;    115;      163]/T);
stop = floor([8.68;  23.44; 62.3;   114.99; 162.7;  240]/T);
slope = zeros(pieces, 1);
yint = zeros(pieces, 1);
...
for i = 1:pieces
    ...
    z = corrected_states(4, start(i):stop(i)).*h.*gear_ratios(1, start(i):stop(i));
    H(1:n, 1) = (start(i):stop(i))*T;
    H(1:n, 2) = ones(n, 1);
...
if(mode_output(7) == 1)
    ...
    for j = 1:num_of_cars
        ...
        e = plot(start(1), z(1), ':', 'Color', [.5 0 1]);
        
        for k = 1:pieces
            t = (start(k):stop(k))*T;
            plot(t, slope(k)*t + yint(k), ':', 'Color', [.5 0 1]);
        end
        
        title(['Car ', int2str(j), ' : Velocity wrt Time']);    xlabel('Time [s]');    ylabel('V [m/s]');
        legend([c e], 'Actual', 'Estimate', 'Location', 'Best');
    end
    ...
    for k = 1:pieces
        t = (start(k):stop(k))*T;
        error = corrected_states(4, start(k):stop(k)).*gear_ratios(1, start(k):stop(k))*h - (slope(k)*t + yint(k));
        per_error = error./(corrected_states(4, start(k):stop(k)).*gear_ratios(1, start(k):stop(k))*h);
        plot(t, per_error*100);
